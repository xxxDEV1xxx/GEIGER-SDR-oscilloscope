#!/usr/bin/env python3
"""
pluto_sweep.py  â€”  PlutoSDR IIO Forensic Sweep Logger  v2
==========================================================
STREAM A â€” IIO frequency sweep (iio_attr)  â†’ sweep_STAMP.jsonl.gz
STREAM B â€” IIO raw IQ power (iio_readdev)  â†’ iq_STAMP.jsonl.gz
...
# iio module bypassed — subprocess only
IIO_TOOLS_PATH = r"J:\iio\libiio-0.19.g5f5af2e\MS64"

def _iio_tool(name):
    import os as _os
    return _os.path.join(IIO_TOOLS_PATH, name)

class _IIOStub:
    class Context:
        def __init__(self, uri=None): pass
        def find_device(self, name): return None
    class Buffer:
...
class LiveJSONL:
    """Appends plain uncompressed JSONL for live tailing by GeigerScope."""
...
# â”€â”€ IIO subprocess wrapper â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ RX port switch â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def switch_rx_port(uri: str, freq_hz: int):
    ...
    if target == _current_rx_port:
        return
    print(f"\n[RX SWITCH] {freq_hz/1e6:.1f} MHz â†’ {target}", flush=True)
...
# â”€â”€ IIO attribute readers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def check_iio_readdev():
    out, rc = iio_run([_iio_tool("iio_readdev.exe"), "--help"], timeout_s=3.0)
    return rc != -2

from ntp_web import get_ntp_info, print_web_time_banner

def print_ntp_banner(ntp_info: dict):
    """Wrapper â€” delegates to ntp_web, ASCII-safe for Windows cp1252."""
    print("\n" + "="*68)
...
# â”€â”€ GzipLog â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
class GzipLog:
...
# â”€â”€ frequency list â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ IQ analysis â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ background IQ sampler for sweep console metrics â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Stream A: sweep â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def run_sweep(uri, freq_list, dwell_ms, settle_ms, anomaly_atten,
              clock, log, live_log, quiet, stop_event, iq_count_fn, sweep_iq):
...
# â”€â”€ Stream B: IQ sampler â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
class IQSampler:
...
def main():
    ...
    if not pluto_info['firmware']:
        print(f"\n[WARN] probe_pluto returned: {pluto_info}")
        print("[WARN] Continuing anyway — Pluto is reachable via ping.")
        pluto_info['firmware'] = 'unknown'
        pluto_info['model']    = 'PlutoSDR'
        pluto_info['serial']   = 'unknown'
    print(f"done.  FW: {pluto_info['firmware']}  SN: {pluto_info['serial']}")

    iq_available = False if args.no_iq else check_iio_readdev()
    if not iq_available:
        print("[INFO] iio_readdev not found â€” Stream B disabled.")
    ...
    sweep_live_path = os.path.join(out_dir, "sweep_live.jsonl")
    sweep_live      = LiveJSONL(sweep_live_path)
    ...
    print(f"  Sweep           : {freq_list[0]/1e6:.3f} â€“ {freq_list[-1]/1e6:.3f} MHz")
    ...
    sweep_thread = threading.Thread(
        ...
        args   = (args.uri, freq_list, args.dwell_ms, args.settle_ms,
                  args.anomaly_atten, clock, sweep_log, sweep_live,
    ...
    try:
        ...
        sweep_live.write(end_rec)
        iq_log.write(end_rec)
        sweep_log.close()
    sweep_live.write({"type": "session_end",
