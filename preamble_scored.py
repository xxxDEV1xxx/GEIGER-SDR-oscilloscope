...
for (plen, prefix), members in prefix_to_sigs.items():
    ...
    if key not in best_per_group or plen > best_per_group[key]['plen']:
        best_per_group[key] = {
            ...
            'count': len(members),
            'members': members,
            'score': len(members) * plen
        }

# sort by score = count * plen descending
groups = sorted(best_per_group.values(), key=lambda x: x['score'], reverse=True)

outfile = 'J:/true-sentinel/preamble_groups_scored.txt'
with open(outfile, 'w', encoding='utf-8') as f:
    f.write('CTW SENTINEL — PREAMBLE GROUPS RANKED BY COUNT x PREFIX LENGTH\n')
    f.write(f'Total groups: {len(groups)}  |  Signatures parsed: {len(sigs)}\n\n')
    for i, g in enumerate(groups):
        f.write('=' * 70 + '\n')
        f.write(f"RANK {i+1}  |  SCORE: {g['score']}  |  PREFIX LEN: {g['plen']}  |  SIGS: {g['count']}\n")
...
print(f'Top 5:')
for g in groups[:5]:
    print(f"  score={g['score']}  count={g['count']}  plen={g['plen']}  prefix=[{','.join(str(x) for x in g['prefix'][:8])}...]")