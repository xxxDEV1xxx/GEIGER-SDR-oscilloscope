...
# Find largest matching preamble groups
# For every pair of signatures, find their longest common prefix
# Then group all signatures that share that prefix

# Build prefix index at every length
from itertools import combinations

# try prefix lengths from longest down to 6
prefix_to_sigs = defaultdict(list)
for s in sigs:
    for plen in range(6, s['len']+1):
        prefix_to_sigs[(plen, s['seq'][:plen])].append(s)

# collect groups with 2+ members, pick longest matching prefix per group
# group by member set
best_per_group = {}
...
# sort by count desc then plen desc
groups = sorted(best_per_group.values(), key=lambda x: (x['count'], x['plen']), reverse=True)

outfile = 'J:/true-sentinel/preamble_groups.txt'
with open(outfile, 'w', encoding='utf-8') as f:
    f.write('CTW SENTINEL — LARGEST MATCHING PREAMBLE GROUPS\n')
    f.write('Sorted: highest member count first, then longest shared prefix\n\n')
...
print(f'Top 5:')
for g in groups[:5]:
    print(f"  count={g['count']}  plen={g['plen']}  prefix=[{','.join(str(x) for x in g['prefix'][:12])}...]")